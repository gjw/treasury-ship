...
**Tools:** axe-core 4.11.1 (via @axe-core/playwright), Lighthouse 12.8.2, Playwright 1.57, source analysis
...
**Methodology:** Google Lighthouse 12.8.2 run via Puppeteer (headless Chrome) on 6 pages.
Desktop preset, no throttling. Authenticated via session cookie for protected pages.
Date: 2026-03-11.

| Page | Score | Failing Audits | Details |
|------|-------|----------------|---------|
| Login | 98/100 | 1 | Missing `<main>` landmark |
| My Week | 97/100 | 1 | Missing `<main>` landmark |
| Dashboard | 96/100 | 1 | Color contrast (9 elements) |
| Issues List | 100/100 | 0 | — |
| Projects List | 96/100 | 1 | Color contrast (5 elements) |
| Documents List | 100/100 | 0 | — |

**Average score: 97.8/100** across 6 pages. Two pages score a perfect 100.

**Failing audit breakdown:**

- **`color-contrast`** — Dashboard (9 elements) and Projects List (5 elements). These align
  with the axe-core findings in Section 2 (Patterns A–C).
- **`landmark`** — Login page and My Week page lack a `<main>` landmark. The app does have
  `<main id="main-content" role="main">` in `App.tsx:541`, but Login renders outside the
  authenticated layout and My Week may not be wrapped in the main landmark at the time
  Lighthouse measures.

**Note:** The Login page "missing main landmark" finding is expected — the login page renders
a standalone centered form without the full app shell. The My Week finding warrants
investigation, as authenticated pages should render within the `<main>` landmark.

---

## 2. Automated Scanner Results (axe-core)
...
## 3. Color Contrast Failures (Detail)
...
| Metric | Baseline |
|--------|----------|
| **Lighthouse accessibility score (avg)** | 97.8/100 across 6 pages |
| **Lighthouse accessibility score (lowest)** | 96/100 (Dashboard, Projects List) |
| **Lighthouse accessibility score (highest)** | 100/100 (Issues List, Documents List) |
